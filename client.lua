...
-- Track the last coupon submission time to prevent spam
local lastCouponTime = 0
...
RegisterNUICallback('submitCoupon', function(data, cb)
    ...
    -- Prevent rapid submissions (cooldown of 5 seconds)
    local currentTime = GetGameTimer()
    if currentTime - lastCouponTime < 5000 then
        cb('cooldown')
        return
    end
    
    -- Update the last submission time
    lastCouponTime = currentTime
