$(function() {
    ...
    function setupCashback() {
        // Prevent multiple submissions flag
        let isSubmitting = false;
        
        $('#apply-coupon').on('click', function() {
            // Don't allow another submission if one is in progress
            ...
            if (couponCode !== '') {
                ...
                setTimeout(function() {
                    ...
                    $('#coupon-code').attr('placeholder', 'Enter your coupon code');
                }, 3000);
            }
        });
        
        // Also handle Enter key in the coupon field
        $('#coupon-code').on('keypress', function(e) {
            if (e.which === 13 && !isSubmitting) { // Enter key and not already submitting
                $('#apply-coupon').click();
                e.preventDefault();
            }
        });
